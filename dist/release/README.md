...
| **영업 종료** | 지정 시간(HH:mm)까지 모든 입출금 차단 |
| **영업 재개** | 즉시 차단 해제 |
| **마스터 비밀번호 변경** | 관리자 모드 진입용 마스터 비밀번호 변경 |
| **카드 PIN 설정** | 카드 UID 입력(또는 태그 자동입력) → 새 PIN 설정 |
| **장비 제어** | (준비 중) |
| **보정** | (준비 중) |
...
5. 카드 태그 → UID 자동 입력됨
6. "카드 PIN 설정" 섹션에서 해당 카드의 PIN 설정
7. 완료 — 이제 Arduino에서 EQ + 마스터 비밀번호로 관리자 진입 가능,
   입출금 거래 시 카드 PIN 검증 가능
```
